...
export default function GroupElasticGallery({ groups }: GroupElasticGalleryProps) {
  ...
  return (
    <>
      ...
      <div className="hidden md:flex w-full rounded-3xl overflow-hidden border border-slate-800 shadow-2xl h-[62vh] min-h-[420px]">
        {groups.map((group, index) => {
          const isActive = group.id === activeId;
          const isWhiteOrLight = group.id === 'blanco' || group.id === 'amarillo';
            ...
            >
              ...
              {!isActive && (
                <div className="absolute inset-0 flex items-center justify-center">
                  <span
                    className="text-[11px] font-bold uppercase tracking-widest text-white/90 whitespace-nowrap select-none drop-shadow-md"
              ...
              {isActive && (
                ...
                >
                  ...
                  {group.slogan && (
                    <p className="text-sm italic text-indigo-200 font-light mb-3">
                      "{group.slogan}"
                    </p>
                  )}
                  ...
                  <div>
                    <Link
                      ...
                      className={`inline-flex items-center gap-2 px-5 py-2.5 rounded-xl text-sm font-extrabold transition-all duration-300 hover:scale-105 hover:shadow-xl ${
                        isWhiteOrLight ? 'text-slate-950' : 'text-white'
                      }`}
                      style={{
                        backgroundColor: group.hex,
        ...
        {groups.map((group, index) => {
          ...
          return (
            <motion.div
              key={group.id}
              onClick={() => setActiveId(group.id)}
              animate={{ height: isActive ? (group.slogan ? 240 : 210) : 64 }}
                  ...
                  >
                    ...
                    <div>
                      <Link
                        href={`/grupos/${group.id}`}
                        onClick={(e) => e.stopPropagation()}
                        className={`inline-flex items-center justify-center gap-2 w-full py-2.5 px-4 rounded-xl text-xs font-extrabold transition-transform active:scale-95 shadow-lg ${
                          isWhiteOrLight ? 'text-slate-950' : 'text-white'
                        }`}
                        style={{
                          backgroundColor: group.hex,
