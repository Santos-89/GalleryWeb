...
// Generador de imágenes mediante IA (Pollinations.ai API CDN)
...
// Dataset dinámico de prueba (60 ítems)
export const MOCK_MEDIA: MediaItem[] = Array.from({ length: 60 }).map((_, index) => {
  ...
  return {
    ...
    url: getAiImageUrl(`${group.name} team activity, outdoor camp action`, width, height, index + 100),
    thumbnailUrl: getAiImageUrl(`${group.name} team activity, outdoor camp action`, 400, 300, index + 100),
    groupColor: group.id,
    aspectRatio,
    likes: 12 + (index * 7) % 85,
    date: '2026-08-10'
